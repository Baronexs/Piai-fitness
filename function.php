...
if(isset($_POST['addnewcustomer'])){
    ...
    // Query untuk menghitung total harga
    $hargatrainer_perbulan = 0; // Inisialisasi dengan nilai default
    ...
    if ($result_trainer && mysqli_num_rows($result_trainer) > 0) {
        ...
        $hargatrainer_perbulan = intval($row_trainer['hargatrainer']); // Pastikan tipe data sesuai jika perlu
    } else {
        echo "Error: Failed to fetch trainer price.";
    }

    // Ambil harga periode jika periode adalah 'Bulanan'
    $hargaperiode_perbulan = 0; // Inisialisasi dengan nilai default
    if ($idperiode === 'Bulanan') {
        ...
        if ($result_periode && mysqli_num_rows($result_periode) > 0) {
            ...
            $hargaperiode_perbulan = intval($row_periode['hargaperiode']); // Pastikan tipe data sesuai jika perlu
        } else {
    ...
    if($addtocustomer){
        ...
        header('Location: customer.php'); // Redirect ke halaman customer.php setelah berhasil
    } else {
        echo "Gagal menambahkan data customer: " . mysqli_error($conn);
        header('Location: customer.php'); // Redirect ke halaman customer.php jika gagal
    }
}
